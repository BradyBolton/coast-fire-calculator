...
function App(props: any) {
  ...
  const currentAgeParam: number = parseInt(
    currentUrl.searchParams.get("ca") ?? "-1"
  );
  const retireAgeParam: number = parseInt(
    currentUrl.searchParams.get("ra") ?? "-1"
  );
  const pmtParam: number = parseFloat(
    currentUrl.searchParams.get("pmt") ?? "-1"
  );
  const rateParam: number = parseFloat(
    currentUrl.searchParams.get("r") ?? "-1"
  );
  const fireNumParam: number = parseFloat(
    currentUrl.searchParams.get("fn") ?? "-1"
  );
  const principalParam: number = parseFloat(
    currentUrl.searchParams.get("p") ?? "-1"
  );
  const pmtBaristaParam: number = parseFloat(
    currentUrl.searchParams.get("pmtb") ?? "0"
  );

  let calcModeParam: string = currentUrl.searchParams.get("mode") ?? "coast";
  calcModeParam =
    calcModeParam === "coast" || calcModeParam === "barista"
      ? calcModeParam
      : "coast";

  // setup local state (coast fire parameters)
  const [currentAge, setCurrentAge] = useState(
    currentAgeParam > -1 ? currentAgeParam : 35
  );
  const [retireAge, setRetireAge] = useState(
    retireAgeParam > -1 ? retireAgeParam : 67
  );
  ...
  const [fireNumber, setFireNumber] = useState(
    fireNumParam > -1 ? fireNumParam : 2000000
  );
  const [principal, setPrincipal] = useState(
    principalParam > -1 ? principalParam : 0
  );
  const [pmtMonthlyBarista, setPmtMonthlyBarista] = useState(pmtBaristaParam);

  const [copiedUrl, setCopiedUrl] = useState(false);
  const [calcMode, setCalcMode] = useState<"coast" | "barista">(
    calcModeParam as "coast" | "barista"
  ); // toggle between coast or barista fire calculations
  ...
  const projections = generateDataSets(
    fireNumber,
    currentAge,
    retireAge,
    rate / 100,
  ...
  const plugin: ChartJSPlugin<"line", CorsairPluginOptions> = {
    ...
    afterDatasetsDraw: (chart: any, args, opts) => {
      ...
      const cursorDate = DateTime.fromMillis(
      ...
      const formattedY = cursorValue.toLocaleString("en", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      });
      const cursorAge = (
        opts.currentAge +
        growthWindow.days / 365
      ).toLocaleString("en", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      });
      ...
      ctx.fillText(
        `(age ${cursorAge})`,
        width / 2 + left,
        top + 25 + 2 * (fontSize + 10)
      );
      ctx.restore();
    }
  };

  const valueCalculator = getValueCalculator(
    projections.result,
    currentAge,
    rate / 100,
    pmtMonthly,
    ...
    calcMode === "barista" ? (
      pmtMonthlyBarista > 0 ? (
        <Typography variant="body2">
          (i.e. after <b>{coastDateStr}</b> you will be able to retire at age{" "}
          <b>{retireAge}</b> as long as you continue saving{" "}
          <b>{`$${pmtMonthlyBarista.toFixed(2)}/mo`}</b>)
        </Typography>
      ) : (
        <Typography variant="body2">
          (i.e. you can start withdrawing{" "}
          <b>{`$${(-1 * pmtMonthlyBarista).toFixed(2)}/mo`}</b> from savings on{" "}
          <b>{coastDateStr}</b> and still retire at age <b>{retireAge}</b>)
        </Typography>
      )
    ) : (
      <Typography variant="body2">
        (i.e. after <b>{coastDateStr}</b> you can halt all retirement
        contributions and still retire at age <b>{retireAge}</b>)
      </Typography>
    );
  ...
  let summaryMessage = (
    <Alert variant="filled" severity="error">
      <Typography variant="body2">FIRE is not currently possible</Typography>
    </Alert>
  );
  if (projections.result.alreadyCoastFire) {
    summaryMessage = (
      <Alert variant="filled" severity="success">
        <Typography variant="body2">
          {calcMode.charAt(0).toUpperCase() + calcMode.slice(1)} FIRE already
          achieved!
        </Typography>
      </Alert>
    );
  } else if (
    projections.result.isPossible &&
    !projections.result.alreadyCoastFire
  ) {
    summaryMessage = (
      <Alert variant="outlined" severity="info">
        <Typography variant="body2">
          In{" "}
          <b>{`${(
            (projections.result.coastFireAge
              ? projections.result.coastFireAge
              : 0) - currentAge
          ).toFixed(2)} years `}</b>
          you can {calcMode} FIRE once you save up{" "}
          <b>
            $
            {projections.postCoastData[0].y.toLocaleString("en-US", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            }) + " "}
          </b>
          on <b>{`${coastDateStr} `}</b> at age{" "}
          <b>{`${
            projections.result.coastFireAge
  ...
  const generatedUrl =
    `https://bradybolton.github.io/coast-fire-calculator/?` +
    `ca=${currentAge}` +
    `&ra=${retireAge}` +
    `&r=${rate}` +
    `&pmt=${pmtMonthly}` +
  ...
  const copiedIcon = copiedUrl ? (
    <FontAwesomeIcon icon={faClipboardPropIcon} className="fa-flip" />
  ) : (
  ...
  const topMessage = useMediaQuery(theme.breakpoints.down("xs")) ? (
    ...
    <Typography variant="subtitle2">
      Watch{" "}
      <Link href="https://www.youtube.com/watch?v=V1ategW3cyk">this video</Link>{" "}
      and check out{" "}
      <Link href="https://walletburst.com/tools/coast-fire-calc/">
        {" "}
        this guy's calculator
      </Link>{" "}
      if you're confused
      ...
      <ScopedCssBaseline>
        <div id="main-container">
            ...
            <Paper sx={{ p: 2 }} elevation={2}>
              <Stack spacing={1}>
                ...
                <Box sx={{ display: "flex", justifyContent: "space-around" }}>
                  <Box>
                    ...
                    <ToggleButtonGroup
                      ...
                      size="small"
                    >
                      <ToggleButton value="coast">Coast FIRE</ToggleButton>
                      <ToggleButton value="barista">Barista FIRE</ToggleButton>
                    </ToggleButtonGroup>
                  </Box>
                  <Box
                    sx={{ display: "flex" }}
                ...
                <Range
                  ...
                  tipDialogText={
                    "Retirement Age is the age you plan to fully retire. In other words, the age you plan to start fully withdrawing money from your retirement savings and stop working."
                  }
                />
                <Divider light />
                <Range
                  ...
                  setTipDialogText={setTipDialogText}
                  tipDialogText={
                    "Initial Principal is the total face value of your retirement assets today. Only include investment assets (not cash). Do not count home equity unless you know what you're doing and you plan to turn that equity into retirement income."
                  }
                />
                <Divider light />
                <Range
                ...
                <Grid alignSelf="center">
                  <Accordion variant="outlined" defaultExpanded>
                    <AccordionSummary
                      expandIcon={
                        <FontAwesomeIcon icon={faAngleDownPropIcon} />
                      }
                      aria-controls="panel1-content"
                      id="panel1-header"
                    >
                      <FontAwesomeIcon icon={faCircleInfoPropIcon} />
                      <Typography sx={{ ml: 1 }}>Withdrawal Info</Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                      <Typography>
                        Make sure to base your FIRE number off of your desired
                        withdrawal rate!
                      </Typography>
                      <List disablePadding>
                        <ListItem disablePadding>
                          <Typography>
                            4% rule:{" "}
                            <b>${(fireNumber * 0.04).toLocaleString()}/yr</b> at{" "}
                            <b>
                              $
                              {((fireNumber * 0.04) / 12).toLocaleString(
                                "en-US",
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2
                        ...
                        <ListItem disablePadding>
                          <Typography>
                            3% rule:{" "}
                            <b>${(fireNumber * 0.03).toLocaleString()}/yr</b> at{" "}
                            <b>
                              $
                              {((fireNumber * 0.03) / 12).toLocaleString(
                                "en-US",
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2
                        ...
                        <ListItem disablePadding>
                          <Typography>
                            2% rule:{" "}
                            <b>${(fireNumber * 0.02).toLocaleString()}/yr</b> at{" "}
                            <b>
                              $
                              {((fireNumber * 0.02) / 12).toLocaleString(
                                "en-US",
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2
                ...
                <Range
                  ...
                  setTipDialogText={setTipDialogText}
                  tipDialogText={
                    "Barista FIRE Contributions is the amount you plan to add (or withdraw) monthly to or from your savings once you achieve barista FIRE. With barista FIRE, you can take a lower paying job, work fewer hours, and even start making small withdrawals from your savings until you achieve true retirement (when you stop working entirely)."
                  }
                />
                <Divider light />
                <Range
          ...
          <div id="graph-container">
            <div id="fixed">
              <Box sx={{ pl: 3, pr: 3 }}>{summaryMessage}</Box>
        ...
        <Dialog open={openTipDialog} onClose={handleClose} maxWidth="md">
          <DialogTitle>{"Explanation"}</DialogTitle>
          <DialogContent>
            <DialogContentText>{tipDialogText}</DialogContentText>
          </DialogContent>
          <DialogActions>
